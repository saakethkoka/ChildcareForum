...
router.get('/savedPost', async (req, res, next) => {
    try {
        ...
        const {DBQuery, disconnect} = await connectToDatabase();
        // const results = await DBQuery('SELECT * FROM savedPost');
        const results = await DBQuery('SELECT *, savedPosts.userID as userID FROM savedPosts LEFT JOIN discussionBoard ON savedPosts.postID = discussionBoard.postID WHERE savedPosts.userID = ' + req.query.curruserID + '')
